...
resource "aws_iam_role_policy_attachment" "stage-cluster-AmazonEKSVPCResourceController" {
...
resource "aws_eks_cluster" "stage" {
  ...
  role_arn = aws_iam_role.stage-cluster.arn
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.stage-cluster-AmazonEKSVPCResourceController,
  ]
}
